...
class PelangganQuotaController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            $record = $model->findOne(['pelanggan_id' => $model->pelanggan_id]);
            if (count($record) > 0) {
                $record->nominal += $model->nominal;
                $record->user_update = Yii::$app->user->identity->id;
            ...
            return $this->redirect(['pelanggan/view', 'id' => $model->pelanggan_id]);
        } else {
